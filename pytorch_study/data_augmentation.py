...
def letterbox(image, size=(640, 640), boxes=None):
    ...
    if boxes is not None:
        ...
        boxes = padded_boxes
    return padded_image, boxes, ratio, dh, h - dh - new_h, dw, w - dw - new_w
...
# 高斯模糊+原图
def gaussian_sharpen(image, boxes, alpha, beta, ksize=(3, 3), gamma=0):
    gaussian = cv2.GaussianBlur(image, ksize, 0)
    img_gaussian_sharpen = cv2.addWeighted(gaussian, alpha, image, beta, gamma)
    return img_gaussian_sharpen, boxes


# 均值模糊+原图
def mean_sharpen(image, boxes, alpha, beta, ksize=(3, 3), gamma=0):
    mean = cv2.blur(image, ksize)
    img_mean_sharpen = cv2.addWeighted(mean, alpha, image, beta, gamma)
...
def rotate_image_and_boxes(image, boxes, angle, rotated_center_x=0.5, rotated_center_y=0.5, zoom=1.0, fill_up=False):
    ...
    rotated_image = cv2.warpAffine(image, rotation_matrix, (image_width, image_height), borderValue=(114, 114, 114))
    rotated_boxes = []
    if boxes is not None:  # 检查boxes是否为None
        for box in boxes:
...
if __name__ == '__main__':
    imagedir = fr'D:\pcb_data\original_ img and yolo_imfo\original img'
    labeldir = fr'D:\pcb_data\original_ img and yolo_imfo\yolo_label_0902'
    ...
    #
    # image, boxes = gaussian_sharpen(datas[0][0], None, 0.4, 0.6, ksize=(3, 3), gamma=0)
    # #
    # image, boxes = mean_sharpen(image, None, 0.5, 0.5, ksize=(3, 3), gamma=0)
    #
    ...
    #
    # select_datas = random.sample(datas, int(len(datas) * 0.4))
    # for data, boxes in select_datas:
    #     uuid_ = uuid.uuid4()
    #     image, boxes = rotate_image_and_boxes(data, boxes, 45, rotated_center_x=0.5, rotated_center_y=0.5,
    #                                           zoom=1.0, fill_up=False)
    #     cv2.imwrite(os.path.join(imagedir, f'{uuid_}.jpg'), image)
    #

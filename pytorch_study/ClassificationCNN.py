...
from tool.base_tool import split_dataset, split_dataset_for_sub_dirs

# 配置日志记录
logging.basicConfig(filename='cnn_training_log.log', level=logging.INFO, format='%(asctime)s - %(message)s')


class ClassificationCNN(nn.Module):
    def __init__(self, num_classes):
        super(ClassificationCNN, self).__init__()
        # self.conv1 = models.vgg16(pretrained=True)
...
if __name__ == "__main__":
    ...
    batch_size, learning_rate, epochs = 16, 1e-4, 10
    # 测试数据集
    ...
    def model_train(data, label, epoch):
        ...
        optim_adam.step()
        # 进度条显示loss和acc
        desc = "train.[{}/{}] loss:{:.4f}, Acc:{:.4f}".format(epoch, epochs, loss, accuracy)
        ...
        for step, (train_data, train_label, train_filename) in enumerate(process_bar):
            ...
            if step == len(process_bar) - 1:
                ...
                for _, (val_data, label, val_filename) in enumerate(val_loader):
                    ...
                    all_val_filenames.append(val_filename.cpu())
        ...
        if epoch == epochs:
            for batch_idx in range(len(all_val_images)):
                ...
                for i in range(len(val_images)):
                    ...
                    img_filename = val_filenames[i].item()
                    # 绘制并保存图像
                    plt.imshow(img)

...
# 配置日志记录
logging.basicConfig(filename='resnet_training_log.log', level=logging.INFO, format='%(asctime)s - %(message)s')


# 定义基本的残差块（Bottleneck block），ResNet-50 中使用 Bottleneck 结构
...
if __name__ == "__main__":
    root_dir = r'/root/pytorch_study/new_bird'
    # 定义目录名称
    ...
    model = ResNet50(subdirectory_count).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.cuda.empty_cache()
    torch.save(model, './resnet_torch_mnist2.pt')
